...
class MultiHeadAttention(nn.Module):
    def __init__(self, nheads, emb_dim):
        ...
        self.Q_heads = [nn.Linear(emb_dim, emb_dim) for i in range(nheads)]
        self.K_head = [nn.Linear(emb_dim, emb_dim) for i in range(nheads)]

    def forward(self, Q, K,V):
        new_qs = [Q_head(Q) for Q_head in self.Q_heads]
        new_ks = [K_head(K) for K_head in self.K_heads]
        head_results = [self.attention(new_qs[i], new_ks[i], V) for i in range(self.nheads)]
        return head_results
