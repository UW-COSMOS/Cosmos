...
RUN conda clean -a
RUN conda install -y pytorch torchvision cudatoolkit=10.1 numpy pandas dask scikit-learn sqlalchemy click beautifulsoup4 tqdm pyarrow tensorboard scikit-image xgboost opencv pdfminer.six tensorboardx gunicorn flask -c pytorch -c conda-forge
