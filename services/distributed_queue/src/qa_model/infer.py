...
from examples.utils_squad import read_squad_examples
...
def convert_to_squad_example(question, paragraph):
    return {'data':[{'title': 'custom_question', 'paragraphs': [{'context': paragraph,
                                   'qas': [{'answers': [],
                                            'question': question,
                                            'id': str(uuid.uuid4())}]
...
def predict(question, paragraph):
...
def infer_qa(query, candidate):
    ...
    return predict(query, candidate)
