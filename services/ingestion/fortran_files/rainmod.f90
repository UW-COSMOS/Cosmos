...
!PBH
      real :: rcritmin = 0.85 ! minimum crit rel hum for non conv clouds
!
!     global scalras
!
...
! PBH NO LONGER NAMELIST VARIABLE
...
!     ==================
      ...
      implicit none
!
! PBH REMOVE RCRIT, ADD RCRITMIN, MOVE RCRIT CALCULATION AFTER NAMELIST INPUT SO IT CAN DEAL WITH RCRITMIN

      namelist/rainpar/kbetta,nprl,nprc,ndca,ncsurf,nmoment,nshallow    &
     &                ,clwcrit1,clwcrit2,pdeep,rkshallow                &
     &                ,pdeepth,nevapprec

      namelist/rainpar_plasim_nml/                                &
               rcritmin,gamma
!
...
!
      ...
      if(mypid==NROOT) then
         read(11,rainpar)
         write (*,'(/," *****************************************")')
         write (*,'(" * RAINMOD ",a29," *")') trim(version)
         write (*,'(" *****************************************")')
         write (*,'(" * Namelist RAINPAR from <puma_namelist> *")')
         write (*,'(" *****************************************")')
         write(*,rainpar)
      endif
      open(unit=56,file='data_PLASIM')
      read(unit=56,NML=rainpar_plasim_nml)
      write(*,rainpar_plasim_nml)
      close(56)

!
      rcrit(:)=MAX(rcritmin,MAX(sigma(:),1.-sigma(:)))
!
...
!
      if(mypid == NROOT .and. ntime == 1) then
       print*,'*********************************************'
       print*,' CPU usage in RAINSTEP (ROOT process only):  '
       print*,'    All routines   : ',time4rain,' s'
       print*,'    Large scale p  : ',time4prl,' s'
       print*,'    Convective p   : ',time4prc,' s'
       print*,'    Cloud formation: ',time4cl,' s'
       print*,'    Dry convection : ',time4dca,' s'
       print*,'*********************************************'
      endif
!
      return
...
!
       ...
       where(dprll(:,jlev) > 0. .and. zt(:) < TMELT)
        dprsll(:,jlev)=dprll(:,jlev)
       endwhere
...
!
       ...
       if(nprint==2) then
        ...
        if(mypid==NROOT) then
         if(jlev==1) print*,'In mklsp:'
         print*,'L= ',jlev,' zt= ',zprf2(nprhor)                        &
     &         ,' zq= ',zprf1(nprhor),' zqsat= ',zprf3(nprhor)
         print*,'L= ',jlev,' dtdt= ',zprf5(nprhor)                      &
     &         ,' dqdt= ',zprf4(nprhor)
         print*,'L= ',jlev,' prl (mm/day)= ',zprf6(nprhor)              &
...
!     ==============
      ...
      use rainmod
      implicit none
      integer jlev,jhor,jlep
      real zfac,zqnew,ztnew,zqsnl,zcor,zdqsdt,zlcp,zrain,zqsa,zzdt,zzdq
...
!
      ...
      if(nprint==2) then
       ...
       if(mypid==NROOT) then
        print*,'In Kuo:'
        print*,'ishallow= ',zprf13(nprhor)
        print*,'ilift= ',zprf5(nprhor),' itop= ',zprf6(nprhor)
        do jlev=1,NLEV
         print*,'L= ',jlev,' dqtot= ',zprf3(nprhor,jlev)                &
     ...
     &     .and. zprf10(nprhor) > 0. .and. zprf13(nprhor) == 0) then
          ...
          print*,'L= ',jlev,' dtdt= ',zzdt,' dqdt= ',zzdq
          print*,'L= ',jlev,' prc (mm/day)= '                           &
...
!     ====================
      ...
      use rainmod
      implicit none
      integer jlev,jlep,jlp,jhor,jlem
      real zkonst1,zkonst2,zrdsig,zfac,zrthl,zzqs,zcor,zzqsp
      real zrh,zrhp,zzf,zztn,zzqn
!
...
!
      if(nprint==2) then
       ...
       if(mypid==NROOT) then
        print*,'In mkshallow:'
        print*,'kshallow= ',zprf7(nprhor),' total= ',SUM(nint(zprf7))
        print*,'ilift= ',zprf5(nprhor),' itop= ',zprf6(nprhor)
        do jlev=1,NLEV
         zztn=zprf1(nprhor,jlev)+zprf3(nprhor,jlev)*deltsec2
         zzqn=zprf2(nprhor,jlev)+zprf4(nprhor,jlev)*deltsec2
         print*,'L= ',jlev,' t= ',zprf1(nprhor,jlev)                    &
     &                    ,' q= ',zprf2(nprhor,jlev)                    
         print*,'L= ',jlev,' tnew= ',zztn,' dtdt= ',zprf3(nprhor,jlev)  &
...
!
      real zclmax,zcca,zccb,zccmax,zccmin
      parameter(zclmax=1.)
