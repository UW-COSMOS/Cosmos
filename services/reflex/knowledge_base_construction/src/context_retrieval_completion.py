...
def run_thread(arguments):
    ...
    # To delete after adding decoding
    rez = metrics.get_ranking(
        arguments["mymodel_probs"],
        arguments["masked_indices"],
        arguments["vocab"],
        is_masked_probs=False,
        ...
        print_generation=arguments["interactive"],
        topk=1,
        return_masked_topk=True
    )
    pred = rez[0]['token_word_form']
    pred = arguments['prediction']
    if is_zsre:
         em, f1, is_error, no_overlap, larger_by_1, larger_by_2, larger_by_3, larger_by_4, larger_by_5_or_more = metrics.calculate_em_f1_zsre(arguments['target'], pred)
    else:
        em, f1, is_error, no_overlap, larger_by_1, larger_by_2, larger_by_3, larger_by_4, larger_by_5_or_more = metrics.calculate_em_f1(arguments['target'], pred)
    msg += "\n" + return_msg
    ...
    return experiment_result, sample_MRR, sample_P, sample_perplexity, msg, em, f1
...
def filter_samples(model, samples, vocab_subset, max_sentence_length, template, is_zsre=True):
    msg = ""
    new_samples = []
    samples_exluded = 0
    for sample in samples:
        ...
        if "obj_label" in sample and "sub_label" in sample:
            ...
            if not is_zsre:
                ...
                if obj_label_ids is not None:
                    print(len(obj_label_ids))
                    #print(model.vocab[obj_label_ids])
...
def main(args, shuffle_data=True, model=None, zsre=True, context_filter=None, single_token=False, inference_top_k=10):
    ...
    for i in tqdm(range(len(samples_batches))):
        ...
        arguments = [
            ...
            for mymodel_probs, original_log_probs, filtered_log_probs, token_ids, masked_indices, label_index, sample in zip(
                mymodel_probs_list,
