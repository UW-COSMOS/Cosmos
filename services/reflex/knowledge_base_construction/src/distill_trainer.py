...
def load_teacher_student_checkpoint(args, trainer):
    ...
    student_checkpoint_path = args.student_restore_file
...
class DistillTrainer(object):
    ...
    def __init__(self, args, task, teacher_model, child_model, criterion, dummy_batch=None, oom_batch=None):
        self.args = args
        self.task = task

        # copy model and criterion to current device
        self._criterion = criterion
        self._model = child_model
        ...
        if self.cuda:
            ...
            self._teacher_model = self._teacher_model.cuda()
    ...
    def load_checkpoint(
        ...
        if os.path.exists(student_filename):
            student_state = checkpoint_utils.load_checkpoint_to_cpu(student_filename)

            # load model parameters
            try:
                self.get_model().load_state_dict(student_state['model'], strict=True)
                if utils.has_parameters(self.get_criterion()):
                    self.get_criterion().load_state_dict(student_state['criterion'], strict=True)
            except Exception:
                raise Exception(
                    'Cannot load model parameters from checkpoint {}; '
                    'please ensure that the architectures match.'.format(student_filename)
                )

            extra_state = student_state['extra_state']
            self._optim_history = student_state['optimizer_history']
            last_optim_state = student_state.get('last_optimizer_state', None)

        if os.path.exists(teacher_filename):
            teacher_state = checkpoint_utils.load_checkpoint_to_cpu(teacher_filename)

            # load model parameters
            try:
                self.get_teacher_model().load_state_dict(teacher_state['model'], strict=True)
            except Exception:
                raise Exception(
                    'Cannot load model parameters from checkpoint {}; '
                    'please ensure that the architectures match.'.format(teacher_filename)
                )
        ...
        if extra_state is not None:
            ...
            print('| no existing checkpoint found {}'.format(student_filename))
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self.model, self.teacher_model, self.criterion, self.optimizer,
                        ignore_grad
                    )
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.teacher_model, self.criterion
                )
            except RuntimeError as e:
