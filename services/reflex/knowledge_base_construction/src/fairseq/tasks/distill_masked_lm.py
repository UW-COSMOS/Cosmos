...
@register_task('distill_masked_lm')
class DistillMaskedLMTask(FairseqTask):
    ...
    def train_step(self, sample, teacher_model, student_model, criterion, optimizer, ignore_grad=False):
        """
        ...
        student_model.train()
        teacher_model.eval()
        loss, sample_size, logging_output = criterion(teacher_model, student_model, sample)
        if ignore_grad:
            loss *= 0
