...
def main(args, shuffle_data=True, model=None):
    ...
    for i in tqdm(range(len(samples_batches))):
        ...
        mymodel_probs_list = []
        print(f'START GPU MEMORY: {torch.cuda.max_memory_allocated()}')
        for sample in samples_b:
            mymodel_probs = run.run(template, sample['sub_label'], ranker, conn, model)
            mymodel_probs_list.append(mymodel_probs)
            print(f'mymodel probs shape: {mymodel_probs.shape}')
            print(f'NOW GPU MEMORY: {torch.cuda.max_memory_allocated()}')

        mymodel_probs_list = torch.stack(mymodel_probs_list)
        torch.cuda.empty_cache()
        print(f'mymodel probs list shape: {mymodel_probs_list.shape}')

        original_log_probs_list, token_ids_list, masked_indices_list, attns = model.get_batch_generation(
            sentences_b, logger=logger
        )
        print(f'original_log_probs_shape: {original_log_probs_list.shape}')
        all_attns.extend(attns)
    ...
    pool.close()
    pool.join()
    with open('attn.pkl', 'wb') as wf:
        pickle.dump(all_attns, wf)
